...
export const LineChartTooltip = ({
  ...
  at,
  ...props
}: LineChartTooltipProps): JSX.Element => {
  ...
  const animatedCursorStyle = useAnimatedStyle(() => {
    ...
    const x1 = atXPosition ?? currentX.value
    if (x1 < elementWidth.value / 2 + xGutter) {
      const xOffset = elementWidth.value / 2 + xGutter - x1
      translateXOffset = translateXOffset - xOffset
    }
    if (x1 > width - elementWidth.value / 2 - xGutter) {
      const xOffset = x1 - (width - elementWidth.value / 2 - xGutter)
      translateXOffset = translateXOffset + xOffset
    }
    ...
    return {
      transform: [
        { translateX: x1 - translateXOffset },
        {
